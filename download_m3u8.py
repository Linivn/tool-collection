...
def get_play_list(url, xpath=None, selected=False):
    ...
    res = get_show_page(url)
    res_tree = etree.HTML(res)

    all_play = {}
    domain_regular = re.compile(r'[a-zA-Z]+://[^\s/]*\.[a-zA-Z0-9]+')
    domain = re.findall(domain_regular, url)[0]
    video_title = res_tree.xpath(xpath.get('video_title'))[0]
    video_title = video_title.replace(' ', '')
    playes = res_tree.xpath(xpath.get('play'))
    ...
    for item in playes:
        ...
        all_play[title] = play_url

    return all_play


def get_m3u8_url(playes):
    """
    获取m3u8链接
    :param playes: 播放列表
    :return:
...
def download_video(url_list):
    ...
    with open('./download_video.bat', mode='w+', encoding='gbk') as file:
        ...
        for k, v in url_list.items():
            if index != 0:
                # 延时2秒，避免N_m3u8DL-CLI生成的日志文件名称相同导致下载报错
...
if __name__ == '__main__':
    ...
    """
    # video_title：剧集名称、episode_title：集数名称、play：播放列表
    # 获取播放链接
    play_list = get_play_list('http://dianying.im/paly-53748-1-1/', xpath={
